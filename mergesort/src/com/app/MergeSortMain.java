...
public class MergeSortMain {
	...
	public static void mergeSort(int arr[],int left,int right) {
		...
		// divide array in two equal patitions
		...
		// compare element from both paritions and copy the smaller one(until any one positon is done)
		while(i<=mid && j<=right) {
		if(arr[i]<arr[j]) {
			temp[k++]=arr[i++];
		}else {
		...
		//copy remaning element from other partition
		while(i<=mid ) 
			temp[k++]=arr[i++];
			
		while(j<= right)
			temp[k++]=arr[j++];
		// overwrite temp arry back to original array
